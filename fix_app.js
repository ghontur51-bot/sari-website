...
if (modalStartIndex !== -1) {
    ...
    // 636:       )}
    // 637:             
    // 638:             <h2 className="text-3xl font-serif font-bold text-[#2A2431] mb-1">Delivery Details</h2>
    ...
    // The modal's closing bracket logic
    const modalContent = code.substring(modalStartIndex, deliveryIndex).trim();
    
    // Remove the modal from inside checkout modal
    code = code.replace(modalContent, '');

    // Place the modal right above the REFINED CART PANEL
